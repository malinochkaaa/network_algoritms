 ...
 * Each instance of the protocol then computes the shortest path tree using the Chandy-Misra algorithm.
 */
public class ChandyMisraProtocol implements CDProtocol {
     ...
     */
    public ChandyMisraProtocol(@SuppressWarnings("unused") String prefix) {
        /* Start in INIT phase */
    ...
    private void init(Linkable lnk, long nodeId) {
        ...
        for (int i = 0; i < lnk.degree(); i++) {
            ...
            graph.add(new CMEdge(nodeId, neighborId, cost, (int) nodeId));
        }
    }
    ...
    private void broadcast(int pid) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            ChandyMisraProtocol tempProtocol = (ChandyMisraProtocol) tempNode.getProtocol(pid);
            /* Copy local graph */
